...
namespace AuctionService.Controllers
{
    ...
    public class AuctionsController : ControllerBase
    {
        ...
        public AuctionsController(AuctionDbContext dbContext
            , IMapper mapper
        ...
        public async Task<ActionResult<AuctionDto>> DeleteAuction(Guid id)
        {
            ...
            await _publishEndpoint.Publish(_mapper.Map<AuctionDeleted>(auction.Id.ToString()));

            var result = await _dbContext.SaveChangesAsync() > 0;

            if (!result) BadRequest("Problem delete entity");
